...
class RenderTool:
    ...
    def execute(self, parameters, messages):
        ...
        time.sleep(readTime)
        #Add message to the results pane in ArcGIS Pro
        ...
        if layername == 'GarageParking':
            ...
            #Update the copy's render to be 'GraduatedColorsRenderer'
            symbology.updateRenderer('GraduatedColorsRenderer')

            #Tell arcpy which field we want to base choropleth map off of
            ...
            layer.symbology = symbology #Key!!!


            #Increment the progressor and change the label; add mesage to the results pane
        ...
        if layername == 'Structures':
            ...
            #Tells acrpy that we want to use "Type" as our unique value
            symbology.renderer.fields= ["Type"]

            #Set the layer's actual symbology to the copy's
            layer.symbology = symbology #Key!!

            #Increment the progressor and change the label; add message to results pane
            arcpy.SetProgressorPosition(maximum)
            arcpy.SetProgressorLabel("saving project...")
            time.sleep(readTime)
            arcpy.AddMessage("saving project...")
